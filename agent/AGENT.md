...
**Тестирование через userbot ОБЯЗАТЕЛЬНО для всех задач!**

Это наш способ убедиться что всё работает в реальном Telegram.

```bash
# 1. Запусти userbot тестер
cd userbot_tester
docker compose --profile testing up -d

# 2. Отправь тестовое сообщение по BDD сценарию
./send.sh "@bot <тестовый запрос>"
# Примеры:
# ./send.sh "Привет!"                    — для архивации
# ./send.sh "@bot сколько сообщений?"    — для AI-агента

# 3. Проверь логи (используй tail!)
tail -50 logs/log_$(date +%Y%m%d).txt

# Или через docker
docker logs --tail=50 telegram-userbot-tester
```

**Что проверить (по BDD сценарию твоей задачи):**
- [ ] Ожидаемое поведение из Gherkin-сценария выполняется
- [ ] Бот ответил (`🤖 БОТ` в логах)
- [ ] Статусы менялись (`✏️ БОТ EDIT`) — для UX задач
- [ ] Файлы отправлены — для задач с файлами
- [ ] Нет ошибок в логах бота (`docker compose logs --tail=50 bot`)
...
### Тестирование
- **Не ставь `[x]` без теста через userbot** — это обязательно!
- Каждая задача должна быть проверена в реальном Telegram
